...
public class DatabaseAttendance {
	...
	public static List<String[]> getTeacherCoursesSections(int teacherId) {
        ...
        try {
            ...
            String sql = """
                SELECT DISTINCT course_name, section, program
                FROM Classes
                WHERE teacher_id = ?
	...
	public static List<Student> getStudentsByProgramSection(String program, String section) {
	    ...
	    String sql = "SELECT student_id, last_name, first_name, middle_name FROM Students WHERE program = ? AND section = ? ORDER BY last_name, first_name";
	    ...
	    try {
	        ...
	        try (Connection conn = dbConn.getConnection();
	            ...
	            try (ResultSet rs = pstmt.executeQuery()) {
	                while (rs.next()) {
	                    int studentId = rs.getInt("student_id");  // Changed to getInt
	                    String lastName = rs.getString("last_name");
	                    String firstName = rs.getString("first_name");
	                    String middleName = rs.getString("middle_name");

	                    // Use Student constructor with int studentId
	...
	public static List<Student> fetchStudentsWithAttendanceForDate(int userId, String courseName, String section, String date) {
	    ...
	    try {
	        ...
	        try (Connection conn = dbConn.getConnection();
	            ...
	            try (ResultSet rs = pstmt.executeQuery()) {
	                while (rs.next()) {
	                    int studentId = rs.getInt("student_id");  // changed here
	                    String lastName = rs.getString("last_name");
	                    String firstName = rs.getString("first_name");
	                    String middleName = rs.getString("middle_name");
	        ...
	        try (PreparedStatement psCheck = conn.prepareStatement(checkSql)) {
	            ...
	            try (ResultSet rsCheck = psCheck.executeQuery()) {
	                if (rsCheck.next()) {
	                    ...
	                    String insertSql = """
	                        INSERT INTO Attendance (enrollment_id, date, status, reason)
	                        VALUES (?, ?, ?, ?)
	                    """;
